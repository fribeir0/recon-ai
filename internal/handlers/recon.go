...
type ReconRequest struct {
	Target string `json:"target"`         // IP, CIDR ou domínio
...
func ReconHandler(c *gin.Context) {
	...
	// 1) Se for IP único (ParseIP != nil e sem "/"), considera direto como ativo
	...
	// 2) Port Scan com Naabu (só nos hosts activos)
	...
	// 3) Service/Version Scan com Nmap (-sV)
	servInfo, nmapErr := services.RunNmapServiceScan(portasAbertas)
	if nmapErr != nil {
		log.Println("[WARN] Erro ao rodar Nmap -sV:", nmapErr)
		// Optamos por prosseguir mesmo que Nmap falhe em algum host
	}
